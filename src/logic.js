...
const taskProto = {
    ...
    getTaskIndex (){
        ...
        projects.forEach( proj => {
            if(proj.title === this.project)
                project = proj;   
        })
        if(this.done){
            for (let i = 0; i < project.doneTaskList.length; i++) {
                const element = project.doneTaskList[i];
                if(element == this)
                    index = i;
            }
        }else if(!this.done){
...
function toggleList(task) {
    ...
    if(task.done === false){
        projects.forEach(proj => {
            if(proj.title == task.project){
                proj.doneTaskList.forEach(instance => {
                    if(instance === task){
                        proj.doneTaskList = proj.doneTaskList.filter(item => item !== instance);
